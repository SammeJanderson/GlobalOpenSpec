...
### Requirement: Project skillshare configuration registers OpenSpec skills

**Reason**: Superseded by global skillshare + global OpenSpec agent home; project `.skillshare/` couples skills to a single repo.

**Migration**: Use global `skillshare init` and the setup script; remove any committed `.skillshare/` if present.

### Requirement: OpenSpec skills remain canonical in repo paths

**Reason**: Skills are machine-global generated artifacts, not repo source.

**Migration**: Run global setup script; remove `.claude/skills/` and `.cursor/skills/` from repos; use `openspec update` on the global agent home to refresh.
